...
//     try {
//       // ❗️ อย่าลืมใส่ ID ของโพสต์ที่จะอัปเดต
//       const userId = localStorage.getItem("user"); 

//       const response = await fetch(`/api/propertypost/${userId}`, {
//         method: 'PATCH',
//         body: formData,
//       });
...
//       console.log('อัปเดตโพสต์สำเร็จ:', result);
...
//                 เลือกรูปภาพอสังหาริมทรัพย์ของคุณ (สูงสุด {MAX_IMAGES} รูป, ขนาดไม่เกิน 5MB ต่อไฟล์)
//               </p>
//               <div
...
function PostUpload() {
  ...
  // ... (ฟังก์ชัน handleAddImages, handleRemoveImage, handleDropImages, handleDragOver ไม่มีการเปลี่ยนแปลง) ...
  const handleAddImages = useCallback(
    (files) => {
      for (const file of files) {
        if (file.size > MAX_IMAGE_SIZE) {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const message = apiError.response?.data?.message || apiError.message || 'เกิดข้อผิดพลาดในการสร้างโพสต์';
      setError(message);
      console.error('API Error:', apiError);
    } finally {
      setIsSubmitting(false);
