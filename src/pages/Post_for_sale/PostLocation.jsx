...
const PostLocation = () => {
  ...
  // 1. โหลดจังหวัดทั้งหมดแค่ครั้งเดียวเมื่อ Component เปิดขึ้นมา
  useEffect(() => {
  ...
  // 2. ฟังก์ชันสำหรับจัดการเมื่อมีการเลือก "จังหวัด"
  const handleProvinceChange = (provinceName) => {
    // รีเซ็ตค่าอำเภอและตำบลในฟอร์มและใน State
    ...
    fetch("https://raw.githubusercontent.com/kongvut/thai-province-data/master/api_amphure.json")
      .then((res) => res.json())
      .then((data) => {
        const filtered = data.filter((a) => String(a.province_id) === String(selectedProvince.id));
        setAmphures(filtered);
      });
  };

  // 3. ฟังก์ชันสำหรับจัดการเมื่อมีการเลือก "อำเภอ"
  const handleDistrictChange = (districtName) => {
    // รีเซ็ตค่าตำบลในฟอร์มและใน State
    form.resetField("Subdistrict");
    setDistricts([]);
    ...
    fetch("https://raw.githubusercontent.com/kongvut/thai-province-data/master/api_tambon.json")
      .then((res) => res.json())
      .then((data) => {
        const filtered = data.filter((d) => String(d.amphure_id) === String(selectedAmphure.id));
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      value={field.value || ""}
                      onValueChange={(val) => {
                        field.onChange(val); // 1. อัปเดตค่าในฟอร์ม
                        handleProvinceChange(val); // 2. เรียกฟังก์ชันจัดการ Logic
                      }}
                    >
                      <SelectTrigger>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      ...
                      onValueChange={(val) => {
                        field.onChange(val); // 1. อัปเดตค่าในฟอร์ม
                        handleDistrictChange(val); // 2. เรียกฟังก์ชันจัดการ Logic
                      }}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      value={field.value || ""}
                      onValueChange={field.onChange} // ตำบลไม่ต้องทำอะไรต่อ แค่อัปเดตค่าก็พอ
